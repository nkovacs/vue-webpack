...
{{indent 2 ~}}      loader: {{string 'css-loader'}},
{{indent 2 ~}}      options: {
{{indent 3 ~}}        minimize: process.env.NODE_ENV === {{string 'production'}},
...
{{indent 4 ~}}          loader: loader + {{string '-loader'}},
...
{{indent 4 ~}}          fallback: {{string 'vue-style-loader'}},
{{indent 3 ~}}        }){{semi}}
{{indent 2 ~}}      }
...
{{indent 2 ~}}      less: generateLoaders({{string 'less'}}),
{{indent 2 ~}}      sass: generateLoaders({{string 'sass'}}, { indentedSyntax: true }),
{{indent 2 ~}}      scss: generateLoaders({{string 'sass'}}),
{{indent 2 ~}}      stylus: generateLoaders({{string 'stylus'}}),
{{indent 2 ~}}      styl: generateLoaders({{string 'stylus'}}),
{{indent 1 ~}}    }{{semi}}
{{indent 0 ~}}  }{{semi}}
...
{{indent 3 ~}}        test: new RegExp({{string '\\.'}} + extension + {{string '$'}}),
{{indent 3 ~}}        use: loader,
{{indent 2 ~}}      }){{semi}}
{{indent 1 ~}}    }){{semi}}
